...
def _build_decay_model():
    ...
    sbio_tools.create_compartment("cell")
    sbio_tools.create_species("A", "cell", value=10.0)
    sbio_tools.create_species("B", "cell", value=0.0)
    sbio_tools.create_parameter("k1", 0.5)
...
def test_tools_repeat_dose_end_to_end():
    ...
    sbio_tools.create_dose(
        "bolus", "A", dose_type="repeat", amount=100.0,
        start_time=5.0, interval=100.0, repeat_count=0)
...
def test_tools_schedule_dose_end_to_end():
    _build_decay_model()
    sbio_tools.create_dose(
        "sched", "A", dose_type="schedule",
        times=[0.0, 3.0, 6.0], amounts=[20.0, 20.0, 20.0])
...
def test_tools_variant_end_to_end():
    ...
    sbio_tools.modify_variant(
        "fast", [{"type": "parameter", "name": "k1", "property": "Value", "value": 0.05}])
...
def test_tools_multi_dose_and_variant():
    _build_decay_model()
    sbio_tools.create_dose(
        "b1", "A", dose_type="repeat", amount=50.0, start_time=2.0, interval=100.0, repeat_count=0)
    sbio_tools.create_dose(
        "b2", "A", dose_type="repeat", amount=50.0, start_time=6.0, interval=100.0, repeat_count=0)
    sbio_tools.create_variant(
